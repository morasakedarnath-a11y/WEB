...
const paramMode = urlParams.get('mode') || (paramTableId ? 'customer' : 'customer');

// Initial State defaults
let state = {
  mode: paramMode, // 'customer' | 'staff'
  activeView: 'menu', // 'menu' | 'kds' | 'admin' | 'qr'
  currentTable: initialTables.find(t => t.id === paramTableId) || { id: 1, name: 'Table 01 - Main Lounge' },
...
// --- REALTIME MULTI-DEVICE CLOUD SYNC ENGINE (FREE ZERO-SETUP SSE STREAM) ---
const CLOUD_CHANNEL = 'bacchus_manipal_tabletop_orders_v1';
const CLOUD_URL = `https://ntfy.sh/${CLOUD_CHANNEL}`;

function sendCloudEvent(type, payload) {
  try {
    fetch(CLOUD_URL, {
      method: 'POST',
      headers: { 'Title': type },
      body: JSON.stringify({ type, payload, timestamp: Date.now() })
    }).catch(err => console.log('Cloud sync post error:', err));
  } catch (e) {
...
// Start Realtime Cloud Sync Stream
initCloudSync();
...
// Check active order for current table
function updateActiveOrder() {
  const tableOrders = state.orders.filter(o => o.tableId === state.currentTable.id && o.status !== 'Delivered');
...
function renderApp() {
  ...
  appContainer.innerHTML = `
    ...
    <main class="container">
      ...
      ${state.mode === 'staff' && state.activeView === 'menu' ? renderCustomerView() : ''}
      ${state.mode === 'staff' && state.activeView === 'kds' ? renderKDSView() : ''}
      ${state.mode === 'staff' && state.activeView === 'admin' ? renderAdminView() : ''}
      ${state.mode === 'staff' && state.activeView === 'qr' ? renderQRView() : ''}
    </main>
  ...
  return `
    <header class="app-header">
      <div class="header-container">
        ...
        ${state.mode === 'staff' ? `
          <nav class="view-nav">
            <button class="view-btn ${state.activeView === 'menu' ? 'active' : ''}" data-view="menu">
              📖 Menu
            </button>
            ...
            <button class="view-btn" id="btn-lock-customer" style="color:var(--accent-red); background:rgba(239,68,68,0.1);">
              🔒 Lock Mode
            </button>
          </nav>
        ` : ''}
        ...
        <div class="header-actions">
          <div class="table-badge" ${state.mode === 'staff' ? 'id="btn-open-scanner"' : ''}>
            <span>📍 ${state.currentTable.name.split('-')[0]}</span>
            ${state.mode === 'staff' ? `<span style="font-size:0.75rem; opacity:0.8;">[Change]</span>` : ''}
          </div>
...
function renderCustomerView() {
  ...
  return `
    ${state.activeOrder ? renderOrderTracker(state.activeOrder) : ''}
  ...
  return `
    <div class="tracker-card">
      <div style="display:flex; justify-content:space-between; align-items:flex-start;">
        <div>
          <span style="color:var(--primary); font-weight:800; font-size:0.8rem; letter-spacing:1px; text-transform:uppercase;">
...
function renderKDSView() {
  ...
  return `
    <div style="margin-bottom:20px; display:flex; justify-content:space-between; align-items:center;">
      <div>
        <h2 style="font-size:1.5rem; font-weight:800;">👨‍🍳 Kitchen Display System (KDS)</h2>
        <p style="color:var(--text-muted); font-size:0.9rem;">Real-time kitchen order tickets synchronized across staff devices.</p>
      </div>
      <button class="btn-add" id="btn-refresh-kds">🔄 Refresh Tickets</button>
    </div>
    ...
    <div class="kds-board">
      ...
      <div class="kds-column">
        ...
        ${preppingOrders.length === 0 ? '<p style="color:var(--text-muted); font-size:0.85rem;">Nothing cooking</p>' : 
          preppingOrders.map(order => renderKDSTicket(order, 'Mark Ready ➔', 'Ready')).join('')}
      </div>
      ...
      <div class="kds-column">
        ...
        ${readyOrders.length === 0 ? '<p style="color:var(--text-muted); font-size:0.85rem;">No items waiting</p>' : 
          readyOrders.map(order => renderKDSTicket(order, 'Mark Delivered 🎉', 'Delivered')).join('')}
      </div>
    </div>
  `;
}
...
function renderAdminView() {
  ...
  return `
    <div style="margin-bottom:20px;">
      ...
      <p style="color:var(--text-muted); font-size:0.9rem;">White-label analytics, menu control, and table service requests.</p>
    </div>

    <div class="stats-grid">
      <div class="stat-card">
        <div style="color:var(--text-muted); font-size:0.85rem; font-weight:700;">TOTAL SALES</div>
      ...
      <div class="stat-card">
        <div style="color:var(--text-muted); font-size:0.85rem; font-weight:700;">AVERAGE ORDER VALUE</div>
        <div class="stat-val">${state.brandConfig.currency}${aov}</div>
        <div style="font-size:0.75rem; color:var(--secondary);">Per table check</div>
      </div>
    </div>
...
function renderStaffPinModal() {
  return `
    <div class="modal-overlay">
      <div class="modal-card" style="padding:24px; max-width:380px;">
        <button class="modal-close" id="btn-close-staff">✕</button>
        <h3 style="font-size:1.2rem; font-weight:800; margin-bottom:6px;">🔐 Staff & Admin Access</h3>
        <p style="color:var(--text-muted); font-size:0.85rem; margin-bottom:16px;">
          Enter Staff Manager PIN to unlock Kitchen KDS & Admin Dashboard.
        </p>

        <div class="form-group">
          <input type="password" class="form-input" id="staff-pin-input" placeholder="Enter PIN (Default: 1234)" autofocus style="font-size:1.2rem; text-align:center; letter-spacing:4px;" />
        </div>
